...
Folder_name="/cptjack/totem/barrylee/temp/" #Image to be Augemented
Extension=".png"
...
def add_light(image, gamma=1.0,Folder_name=Folder_name):
    invGamma = 1.0 / gamma
...
def saturation_image(image,saturation,Folder_name=Folder_name):
...
def contrast_image(image,contrast,Folder_name=Folder_name):
    image = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    image[:,:,2] = [[max(pixel - contrast, 0)
                     if pixel < 190 else min(pixel + contrast, 255)
...
def contrast_brightness_image(image, hue, bri,Folder_name=Folder_name):
    h, w, ch = image.shape  # 获取shape的数值，height和width、通道
    # 新建全零图片数组src2,将height和width，类型设置为原图片的通道类型(色素全为零，输出为全黑图片)
    src2 = np.zeros([h, w, ch], image.dtype)
    dst = cv2.addWeighted(image, hue, src2, 1 - hue, bri)  # addWeighted函数说明如下
    cv2.imwrite(Folder_name + "-contrast_brightness-" + str(4) + Extension, dst)
#
